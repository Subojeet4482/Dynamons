...
# FEATURE: FULL API CONTROL
# Saari settings (pets, strategy, templates) ab API se control hongi.
# Naye endpoints: /config (GET, POST), /history (GET, DELETE)
...
def find_on_screen_adaptive(screen, template_name, threshold=0.8):
    ...
    if not os.path.exists(path):
        # Agar file nahi hai, toh check karo ki template list mein hai
        if template_name in TEMPLATES and "YAHAN-" not in TEMPLATES[template_name]:
            ...
            setup_templates_folder() # Download ki koshish karo
        else:
            return None # Template defined nahi hai
...
def get_elements(screen):
    ...
    # Mera pet element ab config se nahi, balki battle screen se dekho
        ...
        if switch_loc := find_on_screen_adaptive(screen, "switch_button"):
            ...
            if available_pets_on_screen:
                ...
                for slot, pet_data in available_pets_on_screen.items():
                    ...
                    current_priority = PET_PRIORITY.get(slot, 99) # Default priority 99
...
def get_history():
    ...
    try:
        ...
        rows = c.fetchall()
        conn.close()
        # sqlite3.Row ko dict mein convert karo
